...
let zombieIdCounter = 0; // Unique ID generator
...
function renderAchievements() {
    ...
    achievements.forEach(a => {
        ...
        list.innerHTML += `
            <div class="list-item ${unlocked ? 'unlocked' : ''}">
                <div>
...
function renderGunLibrary() {
    ...
    weaponDB.forEach(w => {
        ...
        list.innerHTML += `
            <div class="list-item ${unlocked ? 'unlocked' : ''}">
                <div>
                    <div class="item-title">${unlocked ? w.name : '???'}</div>
                    <div class="item-desc">${unlocked ? (w.type.toUpperCase() + " | DMG: " + w.dmg) : 'Locked'}</div>
                </div>
                <div style="color:${w.color}; font-size:24px;">${unlocked ? '🔫' : '❓'}</div>
            </div>`;
    });
}
...
function launchGame() {
    ...
    players = {};
    players['p1'] = createPlayer('p1', 400, 300, '#3498db');
    if(Network.mode !== 'OFFLINE') players['p2'] = createPlayer('p2', 450, 300, '#e67e22');

    if(Network.mode === 'CLIENT') me = players['p2'];
    else me = players['p1'];
...
function requestRestart() {
    ...
    if(Network.mode === 'HOST') Network.conn.send({ type: 'START' }); 
    launchGame();
}

function createPlayer(id, x, y, color) {
    return { 
        id: id, x: x, y: y, r: 15, hp: 100, maxHp: 100, state: 'ALIVE', 
...
function loop() {
    ...
    if(Network.mode === 'CLIENT') {
        ...
        zombies.forEach(z => {
            if(z.serverX !== undefined && z.serverY !== undefined) {
                // Smoothly slide to the target position received from server
        ...
        Object.values(players).forEach(p => {
            if(p.triggerShoot) { shootGun(p); p.triggerShoot = false; }
        });
    ...
    if(camTarget) {
        camera.x = camTarget.x - canvas.width/2;
        camera.y = camTarget.y - canvas.height/2;
        drawGame();
        updateUI();
    } else {
...
function updatePlayerPhysics(p, isLocal) {
    if(p.state === 'DOWNED') {
        if(p.reviveTimer > 0) {
            p.reviveTimer--;
            if(p.reviveTimer === 0) { 
                p.state = 'ALIVE'; p.hp = p.maxHp; p.hasJug = false; 
    ...
    if(isLocal) {
        ...
        if(mouse.down) {
            if(gun.auto) {
                ...
                if(!mouse.pressHandled) {
                    p.triggerShoot = true;
                    mouse.pressHandled = true; 
                }
            }
        } else {
    ...
    if(p.reloading) {
        p.reloadTimer--;
        if(p.reloadTimer <= 0) {
            let needed = gun.mag - gun.clip; let take = Math.min(needed, gun.ammo);
...
function checkInteractUI() {
    let msg = document.getElementById('interact-msg'); msg.style.display = 'none';
    me.interactionTarget = null;
    if(me.state !== 'ALIVE') return;
    
    // Check REVIVE
    let downedPlayer = Object.values(players).find(p => p !== me && p.state === 'DOWNED');
    if(downedPlayer && Math.hypot(me.x - downedPlayer.x, me.y - downedPlayer.y) < 50) {
        msg.style.display = 'block'; msg.innerText = "[F] REVIVE TEAMMATE";
        me.interactionTarget = { type: 'REVIVE', obj: downedPlayer };
        return;
    }

    let interact = RoomSystem.getNearbyInteractable(me.x, me.y);
    if(interact) {
        msg.style.display = 'block'; msg.innerText = interact.label;
...
function handleInteractAction() {
    if(me.state !== 'ALIVE') return;
    if(Network.mode === 'CLIENT') { Network.sendInteract(); } else { processInteraction(me); }
}
...
function processInteraction(p) {
    ...
    if(teammate && Math.hypot(p.x - teammate.x, p.y - teammate.y) < 50) {
        teammate.state = 'ALIVE'; teammate.hp = teammate.maxHp; teammate.hasJug = false;
        addText(teammate.x, teammate.y, "REVIVED!", "#0f0");
        return;
    }
    ...
    if(interact) {
        ...
        else if(t.type==='WALLBUY' && p.score >= t.obj.price) {
            p.score-=t.obj.price; 
            if(p===me) unlockGun(t.obj.label);
            let ext = p.inventory.find(w=>w.name===t.obj.label);
            if(ext) { ext.ammo=ext.reserve; addText(p.x, p.y, "MAX AMMO", "#fff"); }
            else { let b=weaponDB.find(w=>w.name===t.obj.label); p.inventory.push({...b, clip:b.mag, ammo:b.reserve}); p.weapIdx=p.inventory.length-1; addText(p.x, p.y, b.name, "#fff"); }
        }
        else if(t.type==='BOX' && p.score>=950) {
            p.score-=950; let rnd=weaponDB[Math.floor(Math.random()*weaponDB.length)];
            p.inventory.push({...rnd, clip:rnd.mag, ammo:rnd.reserve}); p.weapIdx=p.inventory.length-1; addText(p.x, p.y, rnd.name+"!", "#0ff");
        }
        else if(t.type==='PERK' && p.score>=t.obj.price && !p.hasJug) { 
            p.score-=t.obj.price; p.hasJug=true; p.maxHp=250; p.hp=250; 
...
function checkAllDead() {
    ...
    if(allDown) {
        let anyoneSelfReviving = Object.values(players).some(p => p.reviveTimer > 0);
        if(!anyoneSelfReviving) {
            gameOver();
        }
    }
}
...
function updateZombies() {
    ...
    zombies.forEach((z, i) => {
        ...
        for(let w of mapData.windows) {
            if(w.boards > 0) {
                if(z.x > w.x - 30 && z.x < w.x + w.w + 30 && z.y > w.y - 30 && z.y < w.y + w.h + 30) {
        ...
        if(attackingWindow) {
            ...
            let a = Math.atan2(target.y - z.y, target.x - z.x);
            let mx = Math.cos(a)*z.speed, my = Math.sin(a)*z.speed;
        ...
        for(let j=i+1; j<zombies.length; j++) {
            ...
            if(dist < 20 && dist > 0) {
                let push = (20 - dist) / 2;
                let ax = ((z.x - z2.x) / dist) * push * 0.5; let ay = ((z.y - z2.y) / dist) * push * 0.5;
...
function checkGameFlow() {
    if(stats.zombiesAlive <= 0 && stats.zombiesToSpawn <= 0 && !stats.changingRound) {
        ...
        setTimeout(() => {
            stats.round++; 
            stats.zombiesToSpawn = Math.floor(6 * Math.pow(1.15, stats.round));
            stats.changingRound = false; 
            
            addText(me.x, me.y-100, "ROUND "+stats.round, "#a83232");
            checkAchievements();

            // --- NEW: RESPAWN LOGIC ---
            // If a player died last round, bring them back now
            Object.values(players).forEach(p => {
                if(p.state !== 'ALIVE') {
                    p.state = 'ALIVE';
                    p.hp = 100;       // Reset Health
                    p.maxHp = 100;    // Reset Max HP (Lose Juggernog)
                    p.hasJug = false; // Ensure flag is false
                    
                    // Teleport them to the survivor so they don't spawn in a locked room
...
function drawScoreboard() {
    ...
    Object.values(players).forEach(p => {
        ...
        tbody.innerHTML += `<tr><td style="color:${p.color}">${p.id.toUpperCase()}</td><td>${p.kills}</td><td>${p.score}</td><td>${status}</td><td>${ping}</td></tr>`;
    });
}

function resetSession() {
    stats = { score: 0, round: 1, zombiesToSpawn: 6, zombiesAlive: 0, frame: 0, sessionKills: 0 };
    zombies = []; bullets = []; particles = []; texts = [];
    zombieIdCounter = 0; // Reset ID counter
    mapData.rooms.forEach(r => r.unlocked = (r.id === 0));
    mapData.windows.forEach(w => w.boards = w.max);
}
...
function gameOver() { 
    ...
    if(document.getElementById('perf-msg')) {
