...
window.GameBalanceConfig = {
    // PLAYER SETTINGS
    PLAYER_BASE_SPEED: 7.0,          // Base moving velocity of survivors
    PLAYER_REGEN_INTERVAL: 60,       // Delay in frames between passive 1 HP healing ticks
    PLAYER_INVINCIBILITY_FRAMES: 20,  // Invincibility frame (i-frame) debounce duration on hit

    // ZOMBIE DAMAGE SETTINGS
    ZOMBIE_BASE_DAMAGE: 25,          // Base damage dealt by standard zombies per bite (was 15)
    ZOMBIE_DAMAGE_ROUND_SCALE: 1.50, // Additional damage bonus scaled per round (was 0.75)
    ZOMBIE_DAMAGE_MAX_CAP: 65,       // Maximum possible damage a standard zombie can deal per hit (was 40)

    // SCORING & XP SETTINGS
    SCORE_ZOMBIE_HIT: 10,            // Points granted for hitting a zombie
    SCORE_ZOMBIE_KILL: 50,           // Points granted for killing a zombie
    XP_ZOMBIE_HIT: 5,                // XP granted on hit
    XP_ZOMBIE_KILL: 25,              // XP granted on kill
    LOBBY_COINS_PER_KILL: 1,         // Lobby coins rewarded per kill

    // WINDOW REPAIR SETTINGS
    SCORE_WINDOW_REPAIR: 10,         // Points awarded per board reconstructed
    XP_WINDOW_REPAIR: 15,            // XP awarded per board reconstructed
    WINDOW_REPAIR_COOLDOWN_MS: 500,  // Cooldown delay between reconstruction clicks

    // WAVE & SPAWN SETTINGS
    MAX_ZOMBIES_ON_SCREEN: 24,       // Maximum active zombies allowed on the map at once
    ROUND_TRANSITION_DELAY_MS: 4000, // Staging transition delay before starting the next round
    POWERUP_DROP_CHANCE: 0.015,      // Random drop probability of power-ups upon a zombie's death
    POWERUP_DURATION_FRAMES: 1800,   // Active duration of power-ups (30 seconds at 60 FPS)
    NUKE_POINTS_REWARD: 400,         // Points awarded to all players upon picking up a Nuke

    // DIFFICULTY SPAWN RATES (Spawning interval delay in frames)
    SPAWN_RATE_EASY: 130,
    SPAWN_RATE_MEDIUM: 100,
    SPAWN_RATE_HARD: 70,

    // DIFFICULTY HP MULTIPLIERS
    HP_MULTIPLIER_EASY: 0.7,
    HP_MULTIPLIER_MEDIUM: 1.0,
    HP_MULTIPLIER_HARD: 1.3,

    // DIFFICULTY WAVE SIZES (Base spawn counts & scaling multipliers)
    WAVE_BASE_EASY: 4,
    WAVE_SCALAR_EASY: 1.10,
    WAVE_BASE_MEDIUM: 6,
    WAVE_SCALAR_MEDIUM: 1.15,
    WAVE_BASE_HARD: 10,
    WAVE_SCALAR_HARD: 1.25,

    // DIFFICULTY SPEED BOUNDS (Min/Max base movement speeds for standard zombies)
    SPEED_MIN_EASY: 1.2,
    SPEED_MAX_EASY: 2.5,
    SPEED_MIN_MEDIUM: 1.8,
    SPEED_MAX_MEDIUM: 4.5,
    SPEED_MIN_HARD: 2.5,
    SPEED_MAX_HARD: 5.5
};
...
function updateGameLogic() {
    ...
    if (Network.mode === 'CLIENT') {
        ...
        for (let i = window.drops.length - 1; i >= 0; i--) {
            let d = window.drops[i]; d.life--;
            let pickedUp = false;
            Object.values(players).forEach(p => {
                if (p.state === 'ALIVE' && Math.hypot(p.x - d.x, p.y - d.y) < 32) { pickedUp = true; applyPowerup(d.type, p); }
            });
...
function updateEnvironmentalHazards() {
                ...
                Object.values(players).forEach(p => {
                    if (p.state === 'ALIVE' && Math.hypot(p.x - t.x, p.y - t.y) < t.r) {
                        if (!p.invincibleTimer || p.invincibleTimer <= 0) {
                            ...
                            if (p.hp <= 0) { p.state = 'DOWNED'; p.reviveTimer = p.hasVigor ? 300 : -1; }
                        }
                    }
                });
    ...
    for (let i = window.groundSmashes.length - 1; i >= 0; i--) {
        let s = window.groundSmashes[i]; s.life--;
        s.r += (s.maxR - s.r) * 0.12;
...
function updatePlayerPhysics(p, isLocal) {
    ...
    if(p.reloading && gun) {
        ...
        if(p.reloadTimer <= 0) { 
            ...
            gun.clip += take; gun.ammo -= take; p.reloading = false; 
        }
    }
}
...
function updateLocalCoopP2(p) {
    if (p.state === 'DOWNED') {
        if (p.reviveTimer > 0) {
            p.reviveTimer--;
            if (p.reviveTimer === 0) {
                p.state = 'ALIVE'; p.hp = p.maxHp; p.hasVigor = false; p.invincibleTimer = 120;
    ...
    if (p2InputConfig === 'keyboard') {
        ...
        zombies.forEach(z => {
            ...
            if (dist < minDist) { minDist = dist; targetZ = z; }
        });
    ...
    if (isReloading && !p2PrevButtons.reload && gun) {
        if (!p.reloading && gun.clip < gun.mag && gun.ammo > 0) {
            p.reloading = true; p.reloadTimer = gun.reload;
...
function shootGun(p) {
    ...
    if(stats.frame - (gun.lastShot||0) >= gun.rpm) {
        ...
        if(gun.clip > 0 || isInfinite) {
            gun.lastShot = stats.frame; p.muzzleFlash = 4;
            ...
            if(Network.mode !== 'CLIENT') {
                for(let i=0; i<pellets; i++) {
                    ...
                    bulletIdCounter++; 
                    bullets.push({ 
                        id: bulletIdCounter, x: p.x, y: p.y, 
...
function spawnShellCasing(x, y, playerAngle) {
    ...
    particles.push({
        x: x, y: y, vx: Math.cos(ejectAngle) * speed, vy: Math.sin(ejectAngle) * speed,
        life: 60 + Math.random() * 30, color: '#f1c40f', type: 'shell',
        angle: Math.random() * Math.PI * 2, rotSpeed: (Math.random() - 0.5) * 0.2, friction: 0.93
    });
}
...
function spawnSparks(x, y, bulletVx, bulletVy) {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        particles.push({ x: x, y: y, vx: Math.cos(a) * speed, vy: Math.sin(a) * speed, life: 10 + Math.random() * 10, color: '#e67e22', type: 'spark' });
    }
}
...
function checkInteractUI() {
    ...
    if(downed) { 
        if (msg) { msg.style.display = 'block'; msg.innerText = "[F] REVIVE " + (downed.name || "TEAMMATE"); }
...
function processInteraction(p) {
    ...
    if(teammate) { 
        teammate.state = 'ALIVE'; teammate.hp = teammate.maxHp; teammate.hasVigor = false; teammate.invincibleTimer = 120;
        addText(teammate.x, teammate.y, "REVIVED (+INVINCIBLE!)", "#0f0"); return; 
    }
        ...
        if(t.type==='WINDOW') { 
            ...
            if (p.score >= cost) {
                ...
                if (hasWeapon) {
                    ...
                    if (ext) { ext.ammo = ext.reserve; addText(p.x, p.y, "MAX AMMO", "#fff"); if (typeof Tutorial !== 'undefined') Tutorial.onAmmoPurchased(); }
                } else {
                    if (p === me) unlockGun(t.obj.label);
                    let b = weaponDB.find(w => w.name === t.obj.label);
                    p.inventory.push({ ...b, clip: b.mag, ammo: b.reserve });
                    p.weapIdx = p.inventory.length - 1; addText(p.x, p.y, b.name, "#fff");
                }
            }
        }
...
function updateZombies() {
    ...
    if (activeMap !== tutorialMapData && stats.zombiesToSpawn > 0 && stats.frame % spawnRate === 0 && stats.zombiesAlive < GameBalanceConfig.MAX_ZOMBIES_ON_SCREEN) {
        ...
        if (valid.length > 0) {
            ...
            let speedMin = GameBalanceConfig.SPEED_MIN_MEDIUM;
            let speedMax = GameBalanceConfig.SPEED_MAX_MEDIUM;
            if (currentDiff === 'easy') {
            ...
            zombies.push(z); stats.zombiesToSpawn--; stats.zombiesAlive++;
        }
    }
    ...
    zombies.forEach((z, i) => {
        ...
        if (!z.hasEntered) {
            ...
            if (closestWin) {
                targetX = closestWin.entryX; targetY = closestWin.entryY;
        ...
        for (let w of activeMap.windows) {
            if (w.boards > 0) {
                if (z.x > w.x - 35 && z.x < w.x + w.w + 35 && z.y > w.y - 35 && z.y < w.y + w.h + 35) { attackingWindow = w; break; }
            }
        }
            ...
            if (attackingWindow) {
                ...
                if (!RoomSystem.checkCollision(testX, testY, false, radiusBuffer)) {
                    ...
                    if (canMoveX && !canMoveY) {
                        ...
                        // 3. Flat-wall direct block: Perpendicular Sideways Sliding (Left/Right)
                        let perpAngle1 = a - Math.PI / 2;
                        let perpAngle2 = a + Math.PI / 2;

                        let px1 = z.x + Math.cos(perpAngle1) * z.speed;
                        let py1 = z.y + Math.sin(perpAngle1) * z.speed;
                        let px2 = z.x + Math.cos(perpAngle2) * z.speed;
                        let py2 = z.y + Math.sin(perpAngle2) * z.speed;
                        ...
                        if (canPerp1 && canPerp2) {
                            ...
                            // 4. Dead-end corner block fallback: 8-Directional Radial Scan
                            let foundPath = false;
                            for (let angleOffset = Math.PI / 6; angleOffset <= Math.PI; angleOffset += Math.PI / 6) {
                                let scanAngle1 = a + angleOffset;
                                let scanAngle2 = a - angleOffset;
                                
                                let testX1 = z.x + Math.cos(scanAngle1) * z.speed;
                                let testY1 = z.y + Math.sin(scanAngle1) * z.speed;
                                if (!RoomSystem.checkCollision(testX1, testY1, false, radiusBuffer)) {
                                    z.x = testX1; z.y = testY1;
                                    foundPath = true;
                                    break;
                                }
                                
                                let testX2 = z.x + Math.cos(scanAngle2) * z.speed;
                                let testY2 = z.y + Math.sin(scanAngle2) * z.speed;
                                if (!RoomSystem.checkCollision(testX2, testY2, false, radiusBuffer)) {
                                    z.x = testX2; z.y = testY2;
                                    foundPath = true;
                                    break;
                                }
                            }
        ...
        Object.values(players).forEach(p => {
            if (Math.hypot(p.x - z.x, p.y - z.y) < 30 && p.state === 'ALIVE') {
                ...
                // Dynamic damage calculations pulled from top settings block
                let baseDmg = GameBalanceConfig.ZOMBIE_BASE_DAMAGE;
                let scaleBonus = Math.floor(stats.round * GameBalanceConfig.ZOMBIE_DAMAGE_ROUND_SCALE); 
                ...
                if (p.hp <= 0) {
                    p.state = 'DOWNED'; p.reviveTimer = p.hasVigor ? 300 : -1;
        ...
        if(!hit) zombies.forEach((z) => {
            ...
            if(!hit && Math.hypot(b.x-z.x, b.y-z.y) < z.r+5) {
                ...
                if (b.type === 'explosive') triggerExplosion(b);
                    ...
                    z.hp -= dmgValue; z.hitTimer = 4;
                    spawnParticles(z.x, z.y, '#800', 3);
                    if (typeof SoundSystem !== 'undefined') SoundSystem.play('zombie_hurt'); 
                    
                    if (Math.random() < 0.45) {
                        window.bloodStains.push({
                            x: z.x + (Math.random() - 0.5) * 12, y: z.y + (Math.random() - 0.5) * 12, r: 4 + Math.random() * 12,
                    ...
                    if (!z.lastHitPointFrame || z.lastHitPointFrame !== stats.frame) {
                        ...
                        if(players[b.ownerId]) { 
                            players[b.ownerId].score += pointsHit; stats.score += pointsHit;
                    ...
                    if(z.hp <= 0) {
                        ...
                        stats.score += pointsKill; stats.zombiesAlive--;
                        
                        if(players[b.ownerId]) { 
                            players[b.ownerId].score += pointsKill; players[b.ownerId].kills++; 
...
function spawnExplosionVisuals(x, y) {
    ...
    for (let j = 0; j < 5; j++) {
        window.bloodStains.push({
            x: x + (Math.random() - 0.5) * 80, y: y + (Math.random() - 0.5) * 80, r: 8 + Math.random() * 20,
...
function applyPowerup(type, picker) {
    ...
    if (type === 'MAX_AMMO') {
        Object.values(players).forEach(p => {
            if (p.inventory) {
                p.inventory.forEach(gun => { gun.ammo = gun.reserve; gun.clip = gun.mag; });
            }
        });
        ...
        zombies.forEach(z => { z.dead = true; stats.zombiesAlive--; if (picker) picker.kills++; });
        zombies = [];
        Object.values(players).forEach(p => { p.score += nukeReward; addText(p.x, p.y - 40, "NUKE! +" + nukeReward, "#e74c3c"); });
...
function resetSession() { 
    ...
    stats = { 
        score: 0, round: 1, zombiesToSpawn: zombiesToSpawnBase, zombiesAlive: 0, frame: 0, 
        sessionKills: 0, selectedMapIdx: currentMapIdx, difficulty: currentDiff
    }; 

    zombies = []; bullets = []; particles = []; texts = []; window.bloodStains = []; zombieIdCounter = 0; 
    window.drops = []; window.doublePointsTimer = 0; window.instaKillTimer = 0;
    
    // Clear hazards & boss pools
    window.activeBoss = null; window.zombieArrows = [];
    window.acidPools = []; window.toxicClouds = []; window.fireZones = []; window.mortarTargets = []; window.groundSmashes = [];
...
function gameOver() { 
    ...
    if (typeof Tutorial !== 'undefined' && Tutorial.isActive) { Tutorial.resetOnDeath(); return; }
...
function returnToLobby() {
    if (Network.mode === 'HOST') {
        try { Network.broadcastToAll({ type: 'RETURN_TO_LOBBY' }); } catch (e) { console.warn(e); }
    }
    goToLobbyScreen();
}
...
function updateUI() {
    ...
    if (me) {
        ...
        if (me.hp < me.lastHp) {
            ...
            if (flash) {
                flash.style.zIndex = "9999"; flash.style.boxSizing = "border-box";
    ...
    ['p1', 'p2', 'p3', 'p4'].forEach(pId => {
        const p = players[pId]; const hud = document.getElementById('hud-' + pId);
        if (hud) {
            if (p) {
